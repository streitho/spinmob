import functions as _fun
import numpy as _numpy
import dialogs as _dialongs
...
def fit(data, model=_models.complete_lorentz_flat_background(), auto_error=1, show_guess=0, show_error=1, plot_all=0, skip_first_try=0, subtract=0, clear_plot=1, invert=0, autofile=1, paths="ask"):
    """
    ...
    while not time_to_quit:
        ...
        # if manual fit file saving, get the location
        if autofile:
            ...
            fit_file = _fun.join(s, "\\")+"\\"+fit_folder+".fits"
...
def interactive_fitting_loop(model, data):
    ...
    while True:
        ...
        # n means "next data set.  Skip this!"
            ...
            if not data.yerror==None: axes.errorbar(data.xdata, data.ydata, "^", color="blue", mec="w", label="data", yerr=data.yerror)
            else:                     axes.plot(    data.xdata, data.ydata, "^", color="blue", mec="w", label="data")

            print "shit", data.yerror
        ...
        if not ask_again:
            ...
            [x, y, ye] = _fun.trim_data(data.xdata, smooth_ydata, data.yerror, [xmin, xmax])
            ...
            if model.skip_next_optimization:
                ...
                if model.auto_trim:
                    ...
                    # trim the data down into new arrays
                    [x, y, ye] = _fun.trim_data(data.xdata, smooth_ydata, data.yerror,
            ...
            if model.plot_all:
                ...
                if data.yerror==None: ye_plot = x_plot*0.0+ye[0]
                else:                 ye_plot = _numpy.array(data.yerror)
            else:
            ...
            _pylab_help.auto_zoom(axes)
            _pylab_help.auto_zoom(axes2)

            # update the plot
            _pylab.draw()
            _pylab_help.raise_figure_window()
            _pylab_help.raise_pyshell()
